...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **热门路线二期（全国 + 北京扩充）**（负责 agent：WorkBuddy 本会话，临时目录 `.tmp/curated-v2/`） | **里程碑 1（2.70.0）与里程碑 2（2.71.0）均已完成提交**：北京 22 条 + 全国 6 条（西安秦岭分水岭/杭州龙井北坡/台州括苍山/深圳梧桐山/成都龙泉山 A 面/昆明西山猫猫箐）全部接入；地区 chips 7 个、卡片网格 + 详情侧栏 + 来源等级/免责声明齐全；几何全由 `scripts/generate-curated-routes.mjs` 从 OSM 产出（含 trunk 放行与隧道白名单），缓存 `.tmp/curated-v2/tmp-overpass/`；数据清单 `docs/热门路线二期规划-数据核实清单.md` | 二期收尾：观察线上反馈；备选池路线（白羊沟/太舟坞/佛爷顶等）与千岛湖环湖（爬升无权威口径）按需再核 |
