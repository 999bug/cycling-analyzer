...
- **真实界面分享卡（v2 起，硬约束）**：分享弹窗有两条出图链路，**都不要删**——
  1. **真实界面**（`ShareStageCard` + `shareStageCapture`，一期仅朋友圈单图）：舞台根节点恒 **1080×1440 布局尺寸**，预览缩放只能挂在弹窗插槽的 `transform` 上，**绝不能缩舞台自身或外层容器的布局尺寸**（Leaflet 的 `getSize()` 读 `clientWidth`，布局被缩会按小视口取瓦片层级，成图糊成一片）；出图时必须 `style: { transform: 'none' }` 清掉缩放并按 1080×1440/2 倍导出，且**先等瓦片稳定**（`waitForStageTiles` 轮询舞台内 `.leaflet-tile-loaded` 计数连续不变，与导出视频同策略），否则成片是空底图；
  2. **极简手绘**（`shareCanvas`，两平台通用）：纯 Canvas 绘制、无底图零网络请求；**快照不可用（旧浏览器/无 canvas 环境）时自动降级到它**，用户必须仍能拿到图。
  舞台通过 `ActivityMap` 的 `staticView` 复用真实地图（隐藏全屏/缩放/底图模式角标/高度拖拽把手、禁用地图交互、高度撑满容器）；静态视图下 `zoomControl` 已关，**不能再挂 `ZoomControlBottomRight`**（`map.zoomControl` 不存在会报错）。底图版权署名照常显示（合规，不随视图模式隐藏）。把底图整屏画进 canvas 的场景（分享出图）必须给瓦片层传 `crossOrigin`（`FallbackTileLayer` → `CachingTileLayer` 落到 img 上），否则跨域瓦片一旦 `drawImage` 即污染画布、`toBlob` 直接抛错。卡片颜色全部取语义令牌，成图跟随站点主题；`modern-screenshot` 走动态 `import()`，只有打开分享弹窗才下载。
