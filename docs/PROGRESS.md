...
> 供后续开发（含 AI agent）继续工作参考。**最后更新：2026-09-16（版本 2.91.2）**——版本功能摘要看 `src/features/changelog/changelogData.ts`（版本倒序，面向用户的权威摘要）；历史实现细节看 `docs/archive/`；整体架构与分层/数据流图看 `docs/架构总览.md`。
>
...
| 🔄 运行中 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血已提交（2.90.1 / 2.90.2）**：① AI 流式首字节/静默超时 + 有限重试；② 瓦片配额协商 + 写失败熔断 + 降级计数时间窗口；③ 迁移锁 CAS 化 + 清表与 done 同事务；④ 导入台账与摘要同事务 + 批量修正改单事务；⑤ db.ts 注释修正 + 构建缺失赛段定义改为告警。**P1 已提交四项**：P1-1 查询规划器 + v8 `localDate` 索引 + 列表索引游标分页（2.91.0）；P1-4 错误边界下沉到路由级与地图区块级（2.91.1）；P1-5 SW 缓存条目上限 + 激活清理（2.91.2）。**§6 基线已实测**（`npm run bench:data-layer`）：列表解出行 1000→50；详情页读 100 点仍解出全部 10 万点（15.4MB）；批量扫描峰值 154MB；**导出分批读放大 11 倍**。**剩余 P1-2/P1-3 未开始**（`activity_chunks` 分片 v9 / 大扫描按片流式） | P1-2 片大小已由基线定为 **2000 点**；取舍已定：迁移按活动原子替换（写 chunks + 删 blobs 同事务），**不保留 blobs 作回退源**（代价与缓解见方案 §8）；**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 / keyset 分页（千级活动下 offset 代价可接受） |
