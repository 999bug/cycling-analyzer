...
> 供后续开发（含 AI agent）继续工作参考。**最后更新：2026-09-16（版本 2.91.0）**——版本功能摘要看 `src/features/changelog/changelogData.ts`（版本倒序，面向用户的权威摘要）；历史实现细节看 `docs/archive/`；整体架构与分层/数据流图看 `docs/架构总览.md`。
>
...
| 🔄 运行中 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血已提交（2.90.1 / 2.90.2）**：① AI 流式首字节/静默超时 + 有限重试；② 瓦片配额协商 + 写失败熔断 + 降级计数时间窗口；③ 迁移锁 CAS 化 + 清表与 done 同事务；④ 导入台账与摘要同事务 + 批量修正改单事务；⑤ db.ts 注释修正 + 构建缺失赛段定义改为告警。**P1-1 已提交（2.91.0）**：`planActivityQuery` 查询规划器（`index-startTime` / `index-localDate` / `full-scan` 三路）+ Dexie v8 `localDate` 索引与启动回填 + 列表改索引游标分页；oracle 补同值排序兜底保证两路径结果可比。**剩余 P1-2/P1-3/P1-4/P1-5 未开始**（`activity_chunks` 分片 v9 / 大扫描按片流式 / ErrorBoundary 下沉 / SW 缓存过期） | P1-2 开工前先按方案 §6 补测第 2、3 项基线（详情页反序列化量 / 热力图峰值内存）——§6 已标注基线尚未实测；**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 |
