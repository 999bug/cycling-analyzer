...
- **AI 接入硬约束（v2.76.0 起）**：BYOK 架构，无后端代理——AI 请求由用户浏览器直连服务商（OpenAI 兼容 `/chat/completions`）。三条红线：
  ...
  3. **未配置 Key 时全部 AI 入口不渲染**（默认关闭，按需开启）；AI 生成的文案只是填入编辑框的草稿，用户可改——不破坏 Share Studio 「模板数字全部可复算」的确定性口径（AI 文案独立于模板，且提示词硬性禁止编造数据）
