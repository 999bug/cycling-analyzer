...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-08（[NF] 骑行记录筛选持久化 + 重置按钮 + 批量重命名弹窗：筛选条件迁移至 activityFilterStore（zustand persist）切页/刷新不丢、仅手动重置；平均功率右侧新增重置/批量重命名入口，模板变量（日期/类型/距离/爬升/序号）+ 实时预览 + 逐条写本地库，作者快照源置灰；版本 2.40.4 → 2.41.0）。
>
