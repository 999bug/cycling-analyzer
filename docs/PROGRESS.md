...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **热门路线二期（全国 + 北京扩充）**（负责 agent：WorkBuddy 本会话，临时目录 `.tmp/curated-v2/`） | **里程碑 1 已完成并随 2.70.0 提交**：北京 22 条（一期 3 + 新增 19，官方/媒体/社区三级来源标注，core 环线注明口径）接入网站；路线图页大改版上线（我的/热门大分区切换 + 地区 chips + 卡片网格 + 详情侧栏，原型 `docs/prototypes/routes-page-redesign.html` 已入库）；几何全由 `scripts/generate-curated-routes.mjs` 从 OSM 产出（缓存 `.tmp/curated-v2/tmp-overpass/`）。**里程碑 2 进行中：全国 6 条接入**（西安分水岭/杭州龙井/台州括苍山/深圳梧桐山/成都龙泉山/昆明西山猫猫箐），清单见 `docs/热门路线二期规划-数据核实清单.md` | 全国 6 条：`CuratedRegionId` 扩地区 + 新数据文件 + `CURATED_REGIONS` 登记 + OSM 几何生成 + chips 自动生效 + 测试（界框按地区登记）→ 版本 2.71.0 |
