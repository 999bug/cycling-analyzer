...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-09（[NF] 反馈窗口升级为双模式：GitHub 预填（需登录）+ 表单收集（无需账户，POST 到国内表单端点/回退托管链接），覆盖无 GitHub 账户用户；版本保持 2.50.0）。
>
