...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-10（[NF] 导出视频体验优化：字幕钩子/数据行支持自定义文案（清空即恢复自动）；录制前新增整屏引导层（讲清授权 + 「开始录制 / 不用授权改用内置绘制」两个明确选择）；录制舞台顶部加醒目状态条（画框外黑边区、不进成片）；合成循环降到 30fps + 画框矩形缓存，录制不再拖慢浏览器；修复「停止共享」后页面卡死——新增 interrupted 信号即时收尾回到原页面；版本 2.60.0。此前 2.59.0 为详情页地图开放底图切换，2.58.0 为导出视频改录制真实页面，2.57.0 为地图模式切换下沉，2.57.1 为记忆目录拆分，2.56.0 为导出视频选项面板）。
>
