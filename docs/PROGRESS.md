...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-11（[IM] 地图瓦片加载提速：瓦片缓存淘汰检查节流（每 16 次写入一次，原每张瓦片全表扫描）+ 淘汰改单次流式遍历不整表载内存；lastAccess 命中刷新节流 60s；`CachingTileLayer` 本地清单检查与 IndexedDB 缓存读取并行（fetch 更早起步）；`index.html` preconnect 预热高德矢量子域 + dns-prefetch 卫星子域；瓦片层 `keepBuffer` 2 → 4。版本 2.63.1。此前 [NF] 2.63.0 为 UI 优化六项：骑行记录筛选无结果空态卡片（含「重置筛选，查看全部 N 条」）；设置页左侧目录改**分区切换**（一次只渲染一个区块，「关于」纳入目录，hash 深链继续可用）；路线图页取消贴边铺满（恢复留白与圆角 + 底部信息区：路线数/累计次数/覆盖里程/当前选中）；**底图模式不再持久化**（默认「正常」，切换仅当前页生效，清理旧键 cycling-map-mode）；导出视频文件名改用活动标题；作者数据「自动」说明改「（默认）」）。
>
