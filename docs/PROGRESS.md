...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **手机端体验优化三批（原型 docs/prototypes/mobile-ux-2026-09-11.html 已审批）** | 批1 核心体验：≤768px 底部 TabBar（仪表盘/记录/统计/路线+更多开抽屉）、记录表格卡片化+筛选收纳、详情页指标 2×2+固定操作底栏、--chart-height 三档、统计/表现 ≤576 降 2×2；批2 扫尾：详情内嵌区块换行、分段/训练计划/年度回顾/热力图断点、地图控件 44px、弹窗热区；批3 全局：tabular-nums、tertiary 对比度、theme-color 动态。全部 ≤768px 内，桌面零影响 | 实施中（本条目完成并提交后移出） |
