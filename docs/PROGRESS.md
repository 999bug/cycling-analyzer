...
| ✅ 已完成 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血 5 项（2.90.1 / 2.90.2）**；**P1 五项全部完成**：P1-1 查询规划器 + v8 `localDate` 索引 + 列表索引游标分页（2.91.0）；P1-4 错误边界下沉（2.91.1）；P1-5 SW 缓存条目上限 + 激活清理（2.91.2）；P1-2 v9 `activity_chunks` 分片 + 按片范围读取 + 后台分片迁移（2.92.0）；P1-3 新增 `iterateRecordBatches` 分批流式并切换 5 处调用点（2.93.0） | **实测收益**（`npm run bench:data-layer`）：详情页读 100 点 15.4MB→315KB（50x）；导出分批放大 11.00x→1.00x；批量扫描峰值 154MB→4.9MB（31x）。**遗留**：`SegmentRecommendations` 的网格聚类需全量输入，未改流式；P0-2 降级恢复探测、P0-4 批量导入事务优化未做（理由见记忆）。**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 / keyset 分页 |
