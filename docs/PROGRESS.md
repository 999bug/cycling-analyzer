...
| 🔄 运行中 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血已提交（2.90.1 / 2.90.2）**：① AI 流式首字节/静默超时 + 有限重试；② 瓦片配额协商 + 写失败熔断 + 降级计数时间窗口；③ 迁移锁 CAS 化 + 清表与 done 同事务；④ 导入台账与摘要同事务 + 批量修正改单事务；⑤ db.ts 注释修正 + 构建缺失赛段定义改为告警。**P1 未开始**（复合索引 + `activity_chunks` 分片） | P1 开工前先按方案 §6 测五项基线（列表耗时 / 详情页反序列化量 / 热力图峰值内存 / 导入事务数 / 构建耗时）；**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 |
