...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-11（[NF] UI 优化六项：骑行记录筛选无结果空态卡片（含「重置筛选，查看全部 N 条」）；设置页左侧目录改**分区切换**（一次只渲染一个区块，「关于」纳入目录，hash 深链继续可用）；路线图页取消贴边铺满（恢复留白与圆角 + 底部信息区：路线数/累计次数/覆盖里程/当前选中）；**底图模式不再持久化**（默认「正常」，切换仅当前页生效，清理旧键 cycling-map-mode）；导出视频文件名改用活动标题；作者数据「自动」说明改「（默认）」；版本 2.63.0。此前 [IM] 2.62.2 为骑行洞察扩充，2.62.1 为架构文档修复，2.61.2 为架构总览文档，2.61.1 为回放录屏技能入库，[NF] 2.61.0 为设置页双栏布局与左侧锚点目录（该目录已于 2.63.0 改为分区切换），[NF] 2.60.0 为导出视频体验优化，2.59.0 为详情页地图开放底图切换，2.58.0 为导出视频改录制真实页面，2.57.0 为地图模式切换下沉，2.56.0 为导出视频选项面板）。
>
