...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **代码审计 P0/P1 修复（按 docs/代码审计报告-2026-09-01.md 路线图）** | 已开干：①`maxOf` 改 for 循环（calculator.ts:169-176）消除超长骑行导入 `RangeError`（V8 实参上限 ~65536，5Hz×5h≈9万点即可超栈）；②ActivityDetailPage `handleCreateSegment` 改坐标防护：coordPoints 为空时拒绝建段（不再 `?? 0` 写几内亚湾 (0,0)）；两处都为零行为变化的小修复 | 跑全量 967/967 + lint/build 绿；版本 2.28.2 → 2.28.3 |
