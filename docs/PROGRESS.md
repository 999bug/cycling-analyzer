...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-09（[NF] 新增全站反馈窗口：右下角悬浮按钮一键提交，经 Cloudflare Worker 代理自动创建 GitHub Issue，版本 2.49.0 → 2.50.0）。
>
...
| UI-11 | 全站反馈窗口（自动建 GitHub Issue） | 中 | ✅ 已实现 | 文件清单：`src/components/FeedbackButton.tsx`（右下角常驻悬浮按钮，复用 CSS 变量、深浅自适应）、`FeedbackModal.tsx`（类型/标题/描述/联系方式 表单，form→loading→success/error 状态机，Esc/遮罩关闭，role=dialog；成功展示新 Issue 链接，失败回退 GitHub 手动新建预填链接）、`feedback.css`、`src/config.ts`（FEEDBACK_ENDPOINT/REPO/TYPES 常量）；`feedback-worker/`（Cloudflare Worker：`index.js` 收 POST、CORS 仅放行站点域名、按 IP 简单限流、输入校验、蜜罐、`GH_TOKEN` 调 GitHub API 建 issue 并按类型打 label，`wrangler.toml` + `README.md` 含部署与申请 Token 文档）。反馈提交经 Worker 代理，GitHub Token 仅存服务端，前端与打包产物均不含密钥。测试：`FeedbackModal.test.tsx` 3 例（成功建 issue / 空标题禁用提交 / 网络失败回退手动链接），全量绿 | 1 commit，版本 2.49.0 → 2.50.0 |
