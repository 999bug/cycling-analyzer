...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-09（[NF] 新增全站反馈窗口：右下角悬浮按钮一键提交，零后端方案直接打开 GitHub 预填 issue 页，国内可直连，版本 2.49.0 → 2.50.0）。
>
...
| UI-11 | 全站反馈窗口（零后端，打开 GitHub 预填 issue 页） | 中 | ✅ 已实现 | 文件清单：`src/components/FeedbackButton.tsx`（右下角常驻悬浮按钮，复用 CSS 变量、深浅自适应）、`FeedbackModal.tsx`（类型/标题/描述/联系方式 表单，form→done 状态机，Esc/遮罩关闭，role=dialog；提交即拼出 GitHub 新建 issue 预填 URL 并打开，弹窗被拦截时仍提供链接兜底）、`feedback.css`、`src/config.ts`（FEEDBACK_REPO / FEEDBACK_TYPES / FEEDBACK_TYPE_LABEL 常量）。零后端：提交时不走自建服务，直接打开 `github.com/<repo>/issues/new?title=&body=&labels=`，自动带入类型 label 与版本号；GitHub 在国内可直连，无需服务端、不含任何密钥。测试：`FeedbackModal.test.tsx` 3 例（提交打开预填页+兜底链接 / 空标题禁用 / 类型带入 label），全量绿 | 1 commit，版本 2.49.0 → 2.50.0 |
