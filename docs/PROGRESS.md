...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-09-08（统计口径修复：距离/均速/最高速度/心率/功率优先设备 session 最终值（佳明 App 同源），GPX 时长改移动时间估算剔除静止段；版本 2.32.0 → 2.32.1）。
>
...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| ✅ 已提交 | **统计口径修复：均速/距离等参数不准**（用户反馈：手动导入的 FIT/GPX 平均速度与其他参数不准；根因——calculateSummary 不用设备 session 最终值：①距离取记录末点累计（设备 GPS 漂移修正/提前停写时与 totalDistance 有差）；②均速由距离÷计时时长自算而非 session.avgSpeed；③GPX 无 session，时长用首末时间差（含红绿灯暂停的 elapsed 口径），均速显著偏低——Strava/佳明 App 均按移动时间显示均速） | ①calculator.ts 设备值优先口径：distance 优先 session.totalDistance → lastDistance → 速度估算回退；avgSpeed 优先 session.avgSpeed（>0 校验，佳明 App 同源）→ distance/duration 回退；maxSpeed 优先 session.maxSpeed；avgHeartRate/maxHeartRate/avgPower/maxPower 同样优先设备值（暂停期 records 平均会混入静息心率拉低均值；设备写 0 视为无效 pickPositive 校验）——avgCadence 保持记录口径（行业口径排除滑行零值，代码注释已说明）；②GPX 移动时间估算 estimateMovingDuration：相邻点位移速度 > 0.5m/s（Strava 同级阈值，GPS 抖动单点 <0.5m/s 不累积）的时间段计入时长，间隔 >30s 视为记录缺失不参与，估算为 0（极稀疏轨迹）回退首末差保底；③真实佳明数据 Node 端验证：FIT 新旧口径距离差 0m/均速差 <0.05km/h（FIT 本就准），主要误差源是 GPX 静止段；④测试：calculator.test 重写基础统计组 6 用例（设备值优先/静止段剔除/稀疏回退/回退链），normalizer.test 合成样例断言适配设备值（distance 2580 vs 末点 2558.5、maxHeartRate 150 vs 截断 149、maxPower 254 vs 253） | 全量 1028/1028 + lint/build 绿；版本 2.32.0 → 2.32.1，changelog 已追加 |
| ✅ 已提交 | **佳明 GDPR 导出包批量导入适配**（用户反馈：佳明批量导出无法导入；根因——用户拿到的是佳明账户 GDPR 全量导出包，FIT 封装在 `DI_CONNECT/DI-Connect-Uploaded-Files/UploadedFiles_*.zip` 内层压缩包中，扫描器只认 `.fit/.fit.gz/.gpx` 文件扩展名，选外层目录/拖 zip 均「未找到可导入的骑行文件」；且佳明摘要 JSON 与 FIT 文件名无键关联——uuidMsb/uuidLsb 对不上文件名 ID，标题只能按活动开始时间匹配） | ①scanner.ts 扫描层 zip 支持：ScanResult 新增 `archives`/`garminJson`/`warnings` 字段 + `isZipFileName` + `emptyScanResult` helper；`expandArchives` 用 fflate 异步 unzip 逐层展开（默认深度 2，覆盖佳明「外层 zip → 内层 UploadedFiles zip → FIT」嵌套；单 zip 条目 ≤5000/解压总量 ≤512MB 防 zip 炸弹；损坏 zip 记 warning 不阻断）；collectFitFiles/scanDirHandle 改走 classifyEntry 统一分拣（zip 登记、`*_summarizedActivities.json` 收进 garminJson）；②新增 `src/features/import/garminExport.ts` 纯函数模块：`isGarminUploadedZipPath`（DI_CONNECT/DI-Connect-Uploaded-Files 路径特征）、`isGarminSummariesName`、`parseGarminSummaries`（`[{summarizedActivitiesExport:[{name,startTimeGmt}]}]` → 秒→活动名 Map，非法 JSON/缺字段静默降级）、`matchGarminTitle`（ISO 开始时间 ±2s 容差最近邻匹配）；③importSources.ts 数据源加 `garmin`（「选择目录（佳明导出）」入口，hint 说明自动解压）；importer.ts ImportOptions 新增 `garminTitles`，标题优先级插位：手动 > Strava CSV > **佳明摘要时间匹配** > GPX 内部名 > 文件名兜底；④ImportPanel：runScan 先 expandArchives 再分流、garminJson 出现即自动解析（拖拽/zip 场景无需选源）、pickFiles 变 async 统一文件/拖拽/目录回退三入口、accept 与拖拽文案补 .zip、解压警告并入 notice 展示；⑤测试：garminExport.test.ts 10 用例 + scannerZip.test.ts 7 用例（内存 zipSync 构造平铺/GDPR 两层嵌套/损坏 zip/深度上限）+ importerGarmin.test.ts 4 用例（真实 FIT 样例时间匹配/容差/手动优先/兜底），用真实佳明 GDPR 包 Node 端模拟拖拽链路端到端验证：85 FIT + 85 摘要条目全部就位 | 全量 1025/1025 + lint/build 绿；版本 2.31.1 → 2.32.0，changelog 已追加 |
| 🔄 运行中 | **代码审计 P0/P1 修复（按 docs/代码审计报告-2026-09-01.md 路线图）** | 已开干：①`maxOf` 改 for 循环（calculator.ts:169-176）消除超长骑行导入 `RangeError`（V8 实参上限 ~65536，5Hz×5h≈9万点即可超栈）——已提交；②ActivityDetailPage `handleCreateSegment` 改坐标防护：filter 用类型谓词收窄到 `{ latitude: number; longitude: number }`，length<2 时拒绝建段（不再 `?? 0` 写几内亚湾 (0,0)），hasTrack 只保证 ≥1 个坐标点，长度<2 仍可能——已提交；③新增 ErrorBoundary（class 组件唯一例外：getDerivedStateFromError + componentDidCatch，hooks 拿不到错误对象）包 Routes，降级页 token 化样式 + 重新加载按钮 + aria-live=assertive——已提交；④parseClient 重构为 WorkerParserHandle（parse+dispose）：60s 单文件超时（Promise.race）主动 terminate + 拒绝未决；OOM/错误/Dispose 共用 rejectAll 路径；importer 批次结束 dispose() 释放含 fitsdk 的 worker——已提交；⑤Dexie 多标签页防死锁：on('blocked') 提示用户关闭其他标签、on('versionchange') 本标签升级后 alert+reload——已提交；⑥classifyParseError 新增 QuotaExceededError 分类→「浏览器存储空间不足」中文文案，importer 写入环节 catch 自动覆盖——已提交；⑦赛段页 N+1 修复：leaderboardWorker 扩展为批量协议（一次提交多赛段+共享 inputs），leaderboardClient 返回 boards Map、计算函数 computeLeaderboardsSync 供主线程同步回退；SegmentsPage 改用 getRecordsByActivityIds 批量取 records、一次 postMessage 提交所有赛段，结构性克隆从 N×160 万对象降到 1×160 万对象——已提交；⑧调查「跨月数据」两个失败用例根因：报告 §3.2 怀疑是时区分裂，实测是测试断言边界漏洞——`DashboardPage.test.tsx`「本月」断言仅检查 `lastSundayInMonth` 边界未检查 `mondayInMonth`（今天 2026-09-04 时 monday=8月31日不在 9月内）；`statisticsPage.test.tsx`「上月范围」断言 1 次实际命中 2 次（act-1 上周日 8月30日 + act-2 上月 15 日 都在 8月内）——修测试断言补全跨月边界，全量 974/974 全绿——已提交；⑨爬升口径：calculateSummary 优先采用 `session.totalAscent/totalDescent`（设备预计算、已对气压量化噪声做平滑过滤），缺失时回退到记录累加——已提交；⑩训练指标校准：a) `avgCadence` 排除滑行 0 值（行业口径 Strava，新加 `averageExcludingZero` 专用函数不影响 `averageOf`）；b) NP 仅满窗（窗口时间跨度 ≥ 30s）才计入 4 次方平均，前/后不满窗数据不再拉低 NP；normalizedPower.test.ts 重写 4 用例手算新值（`已知小数据集`从 NP≈238 → 满窗 300、`滑动窗口滑出`从 NP≈187 → 196.88 → 改用例为 `100/300` 交替满窗均值都 200 → NP=200）；实测 0-29s 零功率 + 30-59s 400W 边界用例 NP 从 222 → 264（更接近真实满窗值，1Hz 采样过渡区仍受混合功率影响）——已提交；⑪P2 重复代码收敛：a) `localDateKey` 抽到 `@/utils/format`（消除 3 处重复实现：calendarData / dashboard.statistics / trainingStatus，calendarData 保持 re-export 兼容老调用方）；b) `formatDurationText` 抽到 `@/utils/format`（消除 2 处重复实现：rideSummary / rideInsights）；c) `shareCard` 硬编码 1609.344 改用 `METERS_PER_MILE` 常量（避免与 settings.ts:143 双源不一致风险）——已提交；⑫复查修复批（2026-09-04 复查 8 个审计修复提交后发现的三处小问题）：a) calculator `calculateSummary` 改单次遍历聚合 max/avg（删除 maxOf/averageOf/averageExcludingZero 三个仅此一处使用的辅助函数，消除 7 个 n 长中间数组分配，语义与逐字段 map+filter 完全一致）；b) db.ts 删除死代码 `cycling-db-blocked` 自定义事件（全仓无监听方，且 window 事件不跨标签、浏览器本会自动对持旧连接的标签触发 versionchange），并让 versionchange 回调先 `this.close()` 主动放行升级再 alert+reload（否则 alert 悬停期间新标签一直卡在 blocked）；c) parseClient dispose 注释矛盾修正（以代码为准：一并拒绝未决请求并销毁 worker）——已提交；⑬修复 CI 部署失败（run 33850809874 在 Test 步骤挂）：根因——⑫的版本提交 2.29.1/2.29.2 未同步追加 changelog 条目，`changelogPage.test.tsx:39` 断言「__APP_VERSION__ = changelog 最新条目」落空（本地跑测试在升版本之前故未暴露，教训：**升版本必须与 changelog 同 commit，且测试在最终版本状态下跑**）；修复——changelogData.ts 头部补 2.29.2 条目（面向用户描述复查修复批），changelogPage 4 用例恢复，全量 974/974 + lint 绿——已提交 | P0/P1/P2 全部完成 + 复查修复批已提交。遗留：P1 3.6 首屏体积（recharts 412kB 仍 modulepreload）、3.2 activityRepository startsWith UTC 真 bug（待专项）、3.5 buildClimbs memo；P0 遗留 exportData 全量入内存（备份通道 OOM 风险） |
| ✅ 已提交 | **在线回放光标丝滑度提升**（用户反馈：地图上的点移动一顿一顿的；根因——光标直接跳到最近记录点坐标，无邻点插值，记录间隔秒级导致逐点跳动；另 10Hz 状态同步经 props 反向写入光标造成微回跳） | ①replayCore.ts 新增 interpolatePositionAt：相邻记录点间按时间比例线性插值（含末段/零跨度防御），光标以帧率连续滑行；②ReplayCursor 播放中由 rAF 循环全权接管光标，syncPosition 仅在非播放态（暂停/拖动）经 effect 对齐，消除 10Hz 回写回跳；③跟随镜头改最小幅度 panBy：仅把越界的光标拉回边距内（原 panTo 居中跳变），动画期间 panningRef 门控不叠加平移；④测试：新增 interpolatePositionAt 4 用例（中点/端点重合/末段钳制/零跨度不除零） | 全量 967/967 + lint/build 绿；版本 2.28.1 → 2.28.2，changelog 已追加 |
| ✅ 已提交 | **在线回放播放卡顿修复**（用户反馈：回放一卡一卡的；根因——rAF 每帧 setProgress 触发全子树 60fps 重渲，且每帧全量 slice+map 重建已走轨迹数组、整条重绘 SVG path） | ①进度架构重构：权威进度存 progressRef 由 rAF 推进，React 状态仅 100ms 节流同步（滑块/HUD/控制条 DOM 不再每帧 reconcile）；②光标命令式更新：ReplayCursor 内自跑 rAF 读 ref → 二分定位 → CircleMarker.setLatLng（不经 React 渲染），高倍速依然平滑；③已走折线抽稀：新增 replayCore.ts buildReplaySkeleton（≤2000 点均匀抽稀保末点），SVG 重绘成本封顶；④纯函数外移：findIndexAtTimestamp/buildReplaySkeleton 移至 src/map/replayCore.ts（react-refresh 导出限制 + 可单测）；⑤顺带修真实 bug：控制条在 MapContainer 内未隔离地图事件——双击按钮误触地图缩放、拖滑块误拖地图（DomEvent.disableClickPropagation/disableScrollPropagation）；⑥防御性修复：rAF 首帧时钟源切换负 dt 防御 + 单帧骑行时间上限 1s（后台切回不跳进度）；⑦测试：新增 tests/map/trackReplay.test.tsx 10 用例（二分边界/骨架抽稀/控制条渲染/倍速切换/拖动联动/fake rAF 确定性推进暂停/缺失字段不伪造） | Playwright 实测 5s 播放采样：301 帧/p95=p99=max=16.8ms/>50ms 帧数 0（稳定 60fps 零卡顿）；版本 2.28.0 → 2.28.1，changelog 已追加 |
...
| ✅ 已提交 | **作者数据地图瓦片预缓存**（用户反馈：高德在线瓦片加载慢） | ①scripts/prefetch-tiles.mjs：读 precomputed/tracks.json 沿作者 81 条轨迹收集 z8-15 唯一瓦片（线性覆盖仅 1674 张 ≈18MB，远小于 bbox 矩形全量），并发受限下载（4 并发+2 重试+断点续传）至 public/author-data/tiles/{z}/{x}/{y}.png + 生成 tiles-manifest.json 清单；②.gitignore 精确化 public/author-data/* 但 !tiles/ 例外入库，CI 无需改动随 dist 发布；③新建 src/map/localTiles.ts（清单懒加载一次并发去重、失败静默降级空集），CachingTileLayer 高德源下 createTile 先查清单命中则 src 直指本地同域路径（零跨域零等待），未命中走原 IndexedDB→在线 链路；④新增 localTiles.test.ts 6 用例；⑤实测 dist 发布含 tiles 且 SW 预缓存未受影响（仍 52 entries/1374KB）。注意：瓦片来自高德公开服务，脚本有并发与总量约束，勿调大或扩大区域 | 全量 924/924 + lint/build 绿；版本 2.23.0 → 2.24.0，changelog 已追加 |
| ✅ 已提交 | **GPX 建段防误导 + GPX 活动解析提速**（用户报告：活动 GPX「戒台寺_潭王路_妙峰山3连击」建段后 0 次穿越；复现定位——该文件为 15.5 小时长距离骑行活动，parseSegmentGpx 取首末点建圆心距仅 643m 的起终点圆，状态机匹配到开头 43 秒的出门晃圈假穿越后被 2.17.0 路径校验正确拦截 → 0 次是防误匹配机制正确工作，根因是「完整活动 ≠ 赛段」的建段语义错位且无任何用户提示；顺带发现 parseGpxActivity 用 doc.getElementsByTagName 在 jsdom 下对 35877 点耗时 152s（live collection 快照 O(N²)）） | ①parseSegmentGpx 返回 durationSeconds（首末 <time> 差，readPointTime 遍历直接子元素取值），segmentMatching 新增 SUSPICIOUS_RIDE_SEGMENT_SECONDS=2h 阈值常量；SegmentsPage handleImportGpx 收集超时长文件名单并在导入反馈中提示「疑似完整骑行而非赛段…建议改为从活动详情页截取设段」（不阻塞建段，入库前剔除 durationSeconds 不落库）；②gpxParser collectTrackPoints 的 getElementsByTagName 换 querySelectorAll（静态 NodeList 线性遍历），childText 手写先序遍历替代逐点 getElementsByTagNameNS 子树搜索——实测 35877 点文件 152s → 7.2s（21 倍，其中 3.2s 为 jsdom DOMParser 本身，浏览器更快）；③临时验证脚本用用户真实文件确认提速与时长识别后删除 | 全量 918/918 + lint/build 绿 |
| ✅ 已提交 | **地图瓦片源默认调换：高德为默认、OSM 降级**（用户需求：境内访问高德稳定快速，OSM 直连经常超时才触发降级已晚） | ①tileSources.ts 调换 TILE_SOURCES 顺序（[amap, osm]），新增语义化辅助收口 25 处硬编码索引语义：tileSourceId 索引↔标识（'amap'/'osm'）、isGcjSource（GCJ-02 纠偏判定）、loadStoredSourceIndex/storeSourceIndex（sessionStorage 读写，语义字符串存储使旧记忆在顺序调换后仍正确解析）；②六个使用点全部替换硬编码判断：ActivityMap/HeatmapPage/RoutesMapPage/CompareSection/SegmentMiniMap/SegmentsPage（含 SegmentsPage 直写字面量 key 处统一改用常量函数）；③测试更新：tileSources.test 默认源断言反转 + 新增 isGcjSource/会话记忆 3 用例，fallbackTileLayer.test 高德源索引同步 | 全量 918/918 + lint/build 绿；版本随同组提交 2.22.0 → 2.23.0 |
| ✅ 已提交 | **性能优化续：recharts 移出主包 + 图表视口内懒挂载**（构建产物诊断：629KB index 主包含 react/react-dom/router/zustand/dexie + recharts 主体——rolldown 将多异步页共享的 recharts 提升进了入口 chunk） | ①vite.config build.rollupOptions.output.advancedChunks 钉住 node_modules/recharts 到独立 chunk：主包 629KB → **269KB（-57%）**，recharts-*.js 422KB（gzip 120KB）由各图表页按需并行加载；②新建通用 src/components/DeferredMount.tsx（复用 hooks/useInView，children 模式零 API 改动，占位块同高度防滚动跳动，jsdom 无 IntersectionObserver 自动退化为立即渲染保测试兼容），应用：仪表盘 TrendChart(minHeight 340)/TrainingStatusSection(360)、详情页 MultiMetricChart+PowerCurveChart 区块(420)；③新增 tests/components/deferredMount.test.tsx 2 用例（无 IO 立即渲染退化兼容/mock IO 视口外占位→进入后挂载）；④index.css 补 .deferred-mount 占位样式（bg-hover + 虚线边框）| 全量 916/916 + lint/build 绿；版本 2.21.0 → 2.22.0，changelog 已追加 |
| ✅ 已提交 | **性能优化专项：fitsdk 重复打包去重 + PWA 预缓存瘦身 + 热力图批量读库**（构建产物实测驱动） | ①fitsdk 去重：Vite worker 构建是独立模块图，manualChunks 无法跨边界共享——改为构建期 resolve.alias（仅 command===build）把主线程降级动态 import '@/fit/worker/parseTask' 指向新建轻量桩 parseTaskStub.ts（抛错兜底，英文消息）；worker 入口用相对路径 './parseTask' 不受 alias 影响，vitest（command=serve）不受影响仍用真实实现；实测 parseTask 384KB 重复副本从 dist 消除。②PWA 预缓存瘦身：globIgnores 排除 parseWorker-*/parseTask*/gpxParser-* 大体积懒加载 chunk + runtimeCaching StaleWhileRevalidate（lazy-assets 缓存名，maxEntries 60）兜底「首次在线使用后离线可用」；预缓存 56 entries/2140KB → 52 entries/**1373KB（-36%）**。③热力图批量读库：activityRepository 接口新增 getRecordsByActivityIds（Dexie anyOf 单次索引查询替代 N 次串行 equals，分组返回 Map，空列表短路；作者仓库补空 Map 实现——作者源走预计算轨迹不逐点拉取），HeatmapPage 替换串行 for-await 循环；新增仓库测试 2 用例（多活动分组/无记录空数组+空列表短路）| 全量 914/914 + lint/build 绿；版本 2.20.0 → 2.21.0，changelog 已追加 |
| ✅ 已提交 | **Strava 标题还原失效修复：activities.csv 兼容 GB18030 编码**（用户反馈：本地导入的 Strava 数据标题显示为日期而非原标题；根因——用户导出的 activities.csv 为 GB18030 编码，csvFile.text() 固定 UTF-8 解码致中文表头变乱码，parseStravaActivitiesCsv 匹配不到「活动 ID/文件名」返回空映射，标题回退链全空后 UI 落到 ActivityListTable 的日期兜底；实测该 CSV 81 个标题+描述+估算功率全丢，author-data 快照 CSV 恰为 UTF-8 故未暴露） | ①stravaExport.ts 新增 decodeTextAuto（UTF-8 BOM 直解 → 无 BOM utf-8 fatal 严格解码 → 失败回退 gb18030，浏览器/Node 原生支持零依赖）与 readTextAuto（Blob 包装）；②ImportPanel.runScan 换用 readTextAuto；③scripts/buildAuthorData.ts loadMetaLookup 同防（readFile utf8 → decodeTextAuto）；④新增 tests/features/import/csvEncoding.test.ts 6 用例（GBK 公开码表字节常量：BOM/合法 UTF-8/纯 ASCII/GB18030 回退 + 与 parseStravaActivitiesCsv 联动端到端语义）；⑤private-fixtures/export_161915040 本地验证：旧实现 0 标题 → 新实现 81 标题全还原，81 文件中 76 个 CSV 有对应行（其余 5 个走文件名兜底，与既有 📌 记录一致）。注意：已入库的错误标题需删除活动重导恢复 | 全量 912/912 + lint/build 绿；版本 2.19.0 → 2.20.0，changelog 已追加 |
...
| ⏸️ 已搁置 | **小程序原生重构**（零域名方案；代码保留在 `feature/miniprogram` 分支，main 不含小程序代码） | 2026-08-26 决策搁置：Phase 0~3 已完成但体验与网站差距大（Canvas 手绘图表 vs Recharts、地图组件封闭、wx.chooseMessageFile 批量导入残废、发布需审核），微信限制与产品核心能力（深度分析/地图/图表/批量导入）根本冲突。移动端入口改走 Web PWA 安装引导（见本表上方 2.27.0 条目）。分支未删除，如重启可从 Phase 4 地图继续 | 无（不再推进） |
...
| ⏸️ 已暂缓 | **功能队列剩余项**（用户 2026-08-26 确认价值不高，暂不推进） | 训练计划生成 ✅ / 表现趋势 ✅ / 每周训练综述 ✅ / 轨迹纠偏 ✅ / 自行车统计 ✅ / 离线地图 ✅ / PWA 离线可用 ✅ / PWA 安装引导 ✅（2.27.0）已完成；剩余四项——目标设定与进度 / 比赛预测 / 路线规划器（画路线导出 GPX）/ 骑行记录 CSV 批量导出——用户反馈「没啥大用」，标记暂缓，后续有需求再启动 | 无（待用户提出新方向） |
