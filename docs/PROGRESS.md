...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **UI 体验改造系列**（2026-08-21 评审立项，任务明细见 §6） | ①UI-1 ✅ `83992ae`；②UI-2 ✅ `9d36a88`；③UI-3 ✅ 已提交；④UI-4 ✅ `a39dc1b`；⑤UI-5 ✅ `26e5ea8`；⑥UI-6 杂项修复 ⏳ 已实现待提交（统计卡「最高功率 0 W」→ `—` 修复（规格 §25 违例）、仪表盘新增「最近骑行」区块（buildDashboardData.recentActivities + RecentRidesSection，填下半屏留白）、日历可点击格子 hover 主题色描边 + 轻微放大 + tooltip 追加「（点击查看当日骑行）」） | 提交 UI-6 → 升版本 2.4.0 → 2.5.0 → push 触发 CI |
