...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **UI 体验改造系列**（2026-08-21 评审立项，任务明细见 §6） | ①UI-1 动态骑行洞察 ✅ 已提交 `83992ae`（8 类洞察纯函数 + RideInsightsSection 组件，测试 19 新增）；②UI-2 详情页报告化 ✅ 已提交 `9d36a88`（rideSummary.ts 类型推断 + RideSummaryBanner + 核心指标精选/折叠，测试 14 新增）；③UI-3 Design Token ✅ 已实现待提交（index.css 语义 token 扩充 + theme/colors.ts 图表调色板统一 + 9 个 CSS 文件 token 化，全量 830/830 + lint/build 绿）；④UI-4 移动端 ⏳；⑤UI-5 隐私可信度 ⏳；⑥UI-6 杂项修复 ⏳ | 提交 UI-3 → 开始 UI-4 移动端重构 |
...
| UI-6 | **杂项修复** | 中 | ⏳ | 见 §6.3 截图实测问题清单，逐项小 commit |
| UI-7 | 高级指标 tooltip | 中 | ⏳ | NP/IF/TSS/CTL/ATL/TSB/FTP/VO2Max 等缩写补"怎么算/怎么理解"说明（详情页区间已有 details 折叠，推广到 Dashboard 训练状态、表现趋势页） | 小 |
