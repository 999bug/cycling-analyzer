...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **热门路线库一期（北京）** | 用户指定方向：周末去哪骑功能路线优先，先上北京热门骑行路线（妙峰山 / 潭王路 / 戒台寺，数据已核实官方口径：19.7km/868m、29.7km/558m、4.96km/213m）。路线几何拟从 OSM 拉取真实道路（WGS-84 入库，渲染走 GCJ-02 投影）；新建 `src/features/curatedRoutes/`（类型 + 北京数据）+ 路线图页「我的路线 / 热门路线」切换，复用地图配色与选中交互。二期规划：全国各地路线（按地区加数据文件即可） | 实施中；数据完整性测试（路径长度 vs 申报里程 sanity check） |
