...
> 供后续开发（含 AI agent）继续工作参考。**最后更新：2026-09-16（版本 2.93.0）**——版本功能摘要看 `src/features/changelog/changelogData.ts`（版本倒序，面向用户的权威摘要）；历史实现细节看 `docs/archive/`；整体架构与分层/数据流图看 `docs/架构总览.md`。
>
...
| 🔄 运行中 | **代码审计遗留项（按 docs/代码审计报告-2026-09-01.md 路线图）** | 审计主体批次与首屏体积项已完成；SimilarRides 全量轨迹驻留问题已在 2.51.11 改为摘要首尾坐标优先，旧活动按需回退；跨时区日期筛选回归测试已在 2.51.12 固定东八区边界；Strava 历史 Token 已在 2.51.13 进入赛段页面时清理；流式导出已在 2.51.14 恢复 2 空格 JSON 层级；批量重命名、删除和纠偏已在 2.51.15 统一本地日期；**审计 P2「ROUTES[下标] 引用 + 双份清单」已在 2.89.2 修复**（`ROUTES` 改具名常量对象、导航清单拆到 `src/layouts/navItems.ts` 复用同一常量，并加 `tests/app/router.test.ts` 防回归；此前「路线图」会打开训练计划页）。剩余修复：工程清理；Vite PWA 插件内部仍有 `inlineDynamicImports` 弃用提示，待插件版本支持后再迁移 | 继续按子项单独提交 |
...
| ✅ 已完成 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血 5 项（2.90.1 / 2.90.2）**；**P1 五项全部完成**：P1-1 查询规划器 + v8 `localDate` 索引 + 列表索引游标分页（2.91.0）；P1-4 错误边界下沉（2.91.1）；P1-5 SW 缓存条目上限 + 激活清理（2.91.2）；P1-2 v9 `activity_chunks` 分片 + 按片范围读取 + 后台分片迁移（2.92.0）；P1-3 分批流式迭代（2.93.0）；**P2 构建与交付**：快照跳过坏文件并告警 + 体积门禁 + e2e/audit 进 CI + 部署回滚入口（2.94.0） | **实测收益**（`npm run bench:data-layer`）：详情页读 100 点 15.4MB→315KB（50x）；导出分批放大 11.00x→1.00x；批量扫描峰值 154MB→4.9MB（31x）。**P2 实测后否掉两项**：增量构建（全量仅 7.0s，缓存 157MB 更慢）、产物 gzip/按年分片（Pages 已做传输压缩、按活动粒度已最优）——理由见方案 §5.1。**遗留**：`SegmentRecommendations` 网格聚类需全量输入未改流式；P2-5 匿名上报未做（涉隐私需单独评审）；P0-2 降级恢复探测、P0-4 批量导入事务优化未做。**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 / keyset 分页 |
...
- **赛段成绩扫描：落库直出 + 增量补扫（v2.89.0 起，硬约束）**：赛段页**不再**每次挂载都全量拉逐点重扫——此前成绩榜只在模块级内存缓存，刷新即失效，进一次要重扫全部活动（几十万点级）是「进赛段要等」的根因。现行流程固定为：① `listEffortsBySegments()` 读 `segment_efforts` 直出渲染（从未扫过且库里无成绩时才留「成绩计算中…」，避免先闪一帧空榜）；② 后台 `refreshBoards()` 用 `src/storage/segmentScanState.ts` 的持久化状态 diff 决定扫不扫、扫哪些——**新增赛段才全量扫，仅活动新增/纠偏时只扫变化的那些活动**，扫完 `mergeEffortsForActivities()` 合并落库并从库里读回合并结果（增量结果只含被扫活动，直接拿来渲染会丢掉未涉及活动的旧成绩）。状态存 `scan_cache`（name `segments-leaderboard`，fingerprint 为结构版本），**活动删除/清空必须同步 `pruneSegmentScanState()` / `clearSegmentScanState()`**（已在 `activityRepository.deleteActivities/deleteAll` 接线），否则同一活动重新导入时 id 与坐标态都没变会被判成「无需扫描」，而它的成绩已在删除时级联清理，永远补不回来。
