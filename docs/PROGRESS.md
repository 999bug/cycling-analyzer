...
> 供后续开发（含 AI agent）继续工作参考。最后更新：2026-08-21（UI 体验改造评审立项，见 §6）。
>
...
| 状态 | 任务 | 进度 | 下一步 |
|---|---|---|---|
| 🔄 运行中 | **小程序原生重构（零域名方案，见 `docs/小程序原生重构方案.md`）** | Phase 0 ✅ 代码完成：3 个同步脚本(sync-core/sync-author-data/decimate-records)已建并跑通 + 原生骨架(app.json tabBar/5页/分包/store) + 包内 require 离线读取 Node 校验通过（作者 81 活动 / 81 抽稀流图 / core 18 个 ts 零浏览器 API）；**UI 真机渲染 + FIT Worker 解析(#3) 待用户在微信开发者工具验收** | 用户微信开发者工具打开 `miniprogram/` 验收仪表盘/活动列表；Phase 1 细化 |
| 🔄 运行中 | **UI 体验改造系列 · 续**（UI-7/UI-8，2026-08-21 下午继续） | ①UI-1~UI-6 ✅ 已推送（版本 2.5.0，commit 见上一行）；②UI-7 高级指标说明 ⏳ 已实现待提交：新增可复用 `src/components/MetricHelp.tsx`（原生 details 折叠），仪表盘训练状态区块补 CTL/ATL/TSB/TSS/NP/IF 六条「怎么算/怎么理解」，表现趋势页补 EF/TSS/NP/移动平均四条；设置页 FTP/VO2Max 估算已有 inline 说明、详情页区间已有 zones-help，无需重复 | 全量测试通过后提交 UI-7 → 评估 UI-8 统计页叙事化 |
...
| UI-8 | 统计页叙事化（P1 页面结构建议） | 低（收益大但改动大） | ⏳ | 按"我进步了吗？/我骑什么路线？/我用什么设备？"重排统计页区块，替代按数据类型平铺 | 大，建议 UI-1~5 完成后再评估 |
