...
> 供后续开发（含 AI agent）继续工作参考。**最后更新：2026-09-16（版本 2.92.0）**——版本功能摘要看 `src/features/changelog/changelogData.ts`（版本倒序，面向用户的权威摘要）；历史实现细节看 `docs/archive/`；整体架构与分层/数据流图看 `docs/架构总览.md`。
>
...
| 🔄 运行中 | **数据层重构**（文档 `docs/数据层重构方案.md`） | **P0 止血已提交（2.90.1 / 2.90.2）**：① AI 流式首字节/静默超时 + 有限重试；② 瓦片配额协商 + 写失败熔断 + 降级计数时间窗口；③ 迁移锁 CAS 化 + 清表与 done 同事务；④ 导入台账与摘要同事务 + 批量修正改单事务；⑤ db.ts 注释修正 + 构建缺失赛段定义改为告警。**P1 已提交五项**：P1-1 查询规划器 + v8 `localDate` 索引 + 列表索引游标分页（2.91.0）；P1-4 错误边界下沉（2.91.1）；P1-5 SW 缓存条目上限 + 激活清理（2.91.2）；P1-2 v9 `activity_chunks` 分片 + 按片范围读取 + 后台分片迁移（2.92.0）。**剩余 P1-3 未开始**（大扫描按片流式迭代） | **§6 基线已实测**：详情页读 100 点 15.4MB→315KB（50x）；**导出分批放大 11.00x→1.00x**；⚠️ **批量扫描 154MB 峰值未变**——`getRecordsByActivityIds` 契约即返回全量，P1-3 必须改调用点才有效；**明确不做**：上服务器 / ORM / 虚拟滚动 / 列式存储 / 账号同步 / keyset 分页 |
