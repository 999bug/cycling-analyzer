...
flowchart TD
    ...
    DAL --> S["存储层 Dexie v8<br/>复合索引 · 分片表 · 配额协商"]
...
- 新增 `listActivities` 快路径：先用索引取候选集，再复用同一套过滤逻辑
- 引入 `ActivityQueryPlanner`：根据 options 决定是否走索引路径

**能走索引的条件**（全部满足才行，否则回退全量）：
- 排序字段是 `startTime`（已有索引）
- 无数值区间筛选（距离/爬升/时长/均速/心率/功率），或虽有但先用时间/类型索引缩小候选集
- 年/月筛选可用 `localDate` 索引（见 D2）
...
### D2 · 新增 `localDate` 字段与复合索引

年/月筛选当前用 `localDateKeyFromIso(a.startTime)` 按**本地日期**字符串前缀匹配（`activityRepository.ts:391,394`），而 `startTime` 是 Unix 秒索引——**时区边界导致两者不可互换**，这是必须新开字段而非复用 `startTime` 索引的原因。
...
this.version(8).stores({
  activities:
    'id, &fingerprint, startTime, activityType, localDate, [activityType+startTime], [localDate+startTime]',
  activity_chunks: '[activityId+seq], activityId',
});
```

**回填策略**：升级事务内**不做**异步重算（会长时间阻塞升级，v5 已为此踩过坑）。改为：
- 导入路径写入时同步填 `localDate`
- 存量数据由后台任务分批回填（复用 `recordsMigration.ts` 的幂等 + 心跳锁模式，并先修掉 D5 的两处缺陷）
- 设置里记 `localDateIndexReady` 标志；未就绪时年/月筛选走全量路径

### D3 · 逐点数据分片

现状 `activity_blobs` 每活动一行、records 为对象数组（`db.ts:425`，`activityRepository.ts:518`）。分片后：

| 项 | 设计 |
| --- | --- |
| 表 | `activity_chunks`，主键 `[activityId+seq]` |
| 片大小 | 5000 点/片（按 P95 活动点数估算，绝大多数活动 1~3 片） |
| 读取 | `getRecords(id, offset, limit)` 只取覆盖 `[offset, offset+limit)` 的片；顺序扫描类场景（热力图）按片流式迭代，不一次 bulkGet |
| 写入 | 导入时按片 `bulkPut`，仍在单一事务内 |
| 兼容 | `activity_blobs` 保留作为回退源，迁移完成前读新表优先、旧表兜底（沿用 v5 既有模式） |
...
| # | 改动 | 验收标准 |
| --- | --- | --- |
| P1-1 | `ActivityQueryPlanner` + v8 复合索引 + `localDate` 回填 | 1000 活动下，无数值筛选的列表首屏从 O(N) 降为索引查询；`queryActivityList` 作为 oracle 的新旧结果一致性测试全绿 |
| P1-2 | `activity_chunks` 分片 + `getRecords` 按片读取 | 详情页读 100 点时反序列化数据量从 ~1.5MB 降到首片量级；`getRecords` 全量读取结果与旧实现逐点相等（契约测试） |
| P1-3 | 热力图/路线图/赛段改按片流式迭代 | 1000 活动扫描时不再一次性 `bulkGet` 全部 blob；峰值内存下降可测量（Chrome performance.memory 或 CDP） |
| P1-4 | ErrorBoundary 下沉到路由级与地图/图表区块级 | 单个图表报错不再整页降级为"重新加载" |
| P1-5 | SW `html-shell` 加 `ExpirationPlugin`；激活时清理非当前版本缓存 | 缓存条目不再随路由数无限增长 |

**回滚**：v8 升级为加表 + 加索引，不删旧表；`activity_blobs` 全程保留，分片可随时停用回退。
...
没有基线就无法证明优化有效。P1 开工前先记录：

1. **列表**：导入 N=1000 条合成活动，`listActivities({limit:50})` 的 `performance.now()` 耗时
2. **详情页**：单活动 10 万点时 `getRecords(id,0,100)` 的耗时与反序列化字节量
3. **热力图**：1000 活动的峰值内存与总耗时
4. **导入**：50 个 FIT 的总耗时与事务数（用 Dexie 的 `db.on('transaction')` 或计数埋点）
5. **构建**：`npm run build:author-data` 全量耗时与产物体积

合成数据用 `node tests/fixtures/generate-samples.mjs` 扩展，**严禁使用 `private-fixtures/` 真实数据做基准或测试**。
...
| 分片迁移中途失败 | 逐点数据不完整 | 必须先完成 P0-3（CAS 锁 + 同事务清理）再启动；旧 `activity_blobs` 全程保留作回退源 |
| 时区/本地日期语义变更 | 年/月筛选结果变化 | `queryActivityList` 作为 oracle 的新旧一致性测试，覆盖跨时区与东八区边界（沿用 2.51.12 的东八区回归用例） |
| 分片大小选择不当 | 小活动片数过多 / 大活动单片仍过大 | 先按现有数据分布统计点数分位数再定 5000 这个值，不要拍脑袋 |
