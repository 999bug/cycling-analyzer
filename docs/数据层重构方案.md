...
| P1-2 | `activity_chunks` 分片（v9）+ `getRecords` 按片读取 | 详情页读 100 点时反序列化数据量从 ~1.5MB 降到首片量级；`getRecords` 全量读取结果与旧实现逐点相等（契约测试） |
...
**回滚**：v8 升级为加索引（不删表不改列），旧版应用打开同一库只是不用新索引，功能不受影响；`activity_blobs` 全程保留，分片可随时停用回退。
...
> ⚠️ **当前进度**：P1-1 已落地，但**上述基线尚未实测**——当时优先做了正确性修复（P0）。P1-1 的收益是结构性可证的（分页从「取全量再切」变为索引游标只读当前页，IO 与内存都从 O(N) 降到 O(page)），但没有数字佐证。
> **P1-2 / P1-3 开工前必须先补第 2、3 项基线**，否则「分片到底省了多少」无法证伪，容易做成心理安慰。
