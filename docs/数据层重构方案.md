...
| P1-3 ✅ | 热力图/路线图/赛段改分批流式迭代 | 已落地（v2.93.0）：新增 `iterateRecordBatches`（默认 16 活动/批），6 处调用点中 **5 处**切换到流式；**批量扫描峰值 154MB → 4.9MB（31x ↓）**，累计解出总量不变（§6.2b） |
...
### 6.2c P1-3 落地后：批量扫描的峰值（v2.93.0 实测）

新增 `iterateRecordBatches`（默认 16 活动/批）后，同一场景（500 活动 × 2000 点）：

| 口径 | 旧路径 `getRecordsByActivityIds` | 新路径 `iterateRecordBatches` |
| --- | --- | --- |
| 累计解出点数 | 1,000,000 点 | 1,000,000 点（**总量不变**，本就该相同） |
| **峰值同时驻留** | 1,000,000 点 / 154.0 MB | **32,000 点 / 4.9 MB** |

**峰值降 31.3x**。这才是 P1-3 要买的东西——总量不变是对的，降到 154MB 以下才是目的。

已切换的 5 处调用点：热力图、路线图、赛段页（按批落库，`mergeEffortsForActivities` 以 activityIds 为作用边界，各批不相交故等价）、赛段详情（累加后一次性整体替换）、赛段创建预览（纯计数累加）。

**仍留在旧路径的 1 处**：`SegmentRecommendations` 的网格聚类 `mineSegmentCandidates(inputs)` 需要**全部**活动参与才能成网格，无法按批拆分；除非改成「按批产出候选再全局去重」的算法，否则改不动。该项记入遗留清单。
