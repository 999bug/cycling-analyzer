...
## 提交推送时的记忆同步（2026-09-14 用户定）

- **每次提交推送前，把 `.workbuddy/memory/` 下的改动文件（当日日志、主题文件）一并 `git add` 进同一次提交**，不要遗留在工作区。
- 脱敏红线照旧：写入 memory 前已脱敏（相对路径/占位符），提交前不需要额外处理，但不得把 `LOCAL-ONLY.md`、`.tmp/` 等未入库文件带进来。
- 多会话并行时只提交自己的改动：逐路径 `git add`，发现别人会话的半成品（如进行中的 src 改动）不要顺手卷进提交。

## push 策略（2026-09-14 用户定：攒批推送）

- **多次本地提交后一次性 push**：功能批次内只做本地 commit（含 memory 同步），攒到批次收尾（CI 验证前）再统一 push；不逐 commit 推送。
- 推送前跑本地全量测试，推完照旧盯 CI 至绿。
